function nlp = create_relaxation_NLP(self, OCP)
% formulate a relaxation based NLP
%
...
%            with mu: nonnegative relaxation parameter
...
switch self.relaxation_problem
    case 'Scholtes'
        ...
        Dim.c_Node = cumsum([OCP.Dim.lambda, OCP.Dim.lambda, eta_Dim]);
        Dim.c = size(c, 1);
    case 'Lin_Fukushima'
        c_stage = [...
            (1/mu)^2 - LAMBDA .* ETA;...
            (LAMBDA + 1/mu) .* (ETA + 1/mu) - (1/mu)^2];
        c = reshape(c_stage, 2 * OCP.Dim.lambda * OCP.nStages, 1);
        Dim.c_Node = cumsum([OCP.Dim.lambda, OCP.Dim.lambda]);
        Dim.c = size(c, 1);
end
